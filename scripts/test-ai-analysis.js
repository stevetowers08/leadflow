...
const testMessages = [
  ...
  },
  {
    ...
    name: 'Gregg McCallum',
    message:
      "Hi Daniel, thanks for the reach out. I'm not the correct person inside Nearmap, as it turns out I've recently resigned and starting my next role in a few weeks. Happy to connect in any case.",
  },
];
...
testMessages.forEach((person, index) => {
  ...
  console.log(
    `   Analysis: ${analysis.replyType} (${(analysis.confidence * 100).toFixed(1)}% confidence)`
  );
...
const notInterested = analyses.filter(
  a => a.replyType === 'not_interested'
).length;
