...
const testCases = [
  {
    ...
    message: "Hi Daniel, thanks for reaching out. I'm interested in learning more about your recruitment services. Can we schedule a call?",
    channel: "email",
    expectedType: "interested"
  },
  {
    name: "LinkedIn Reply Test", 
    message: "Thanks for the connection request. I'm not currently looking for new opportunities, but I appreciate the outreach.",
    channel: "linkedin",
    expectedType: "not_interested"
  },
  {
    name: "Email Maybe Test",
    message: "Hi, I received your message. I'm not sure if we're the right fit right now, but I'll keep your information for the future.",
    channel: "email", 
    expectedType: "maybe"
  }
];
...
testCases.forEach((testCase, index) => {
  ...
  const emoji = analysis.replyType === 'interested' ? '😊' : 
                analysis.replyType === 'not_interested' ? '😞' : '😐';
  
  console.log(`   Analysis: ${emoji} ${analysis.replyType} (${(analysis.confidence * 100).toFixed(1)}% confidence)`);
  console.log(`   Reasoning: ${analysis.reasoning}`);
  console.log(`   Expected: ${testCase.expectedType} ${analysis.replyType === testCase.expectedType ? '✅' : '❌'}`);
  console.log('');
});

console.log('✅ Both Email and LinkedIn reply analysis working!');
console.log('\n📊 Summary:');
console.log('📧 Email replies: Automatically analyzed when last_reply_message is updated');
console.log('💼 LinkedIn replies: Automatically analyzed when linkedin_responded = true');
