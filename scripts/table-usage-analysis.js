...
async function analyzeTableUsage() {
  ...
  const codeReferencedTables = [
    'people', 'companies', 'jobs', 'conversations', 
    'email_threads', 'email_messages', 'dashboard_metrics',
    'People', 'Companies', 'Jobs' // Capitalized versions
  ];
  ...
  for (const tableName of codeReferencedTables) {
    try {
      ...
      if (!error) {
        ...
        console.log(`❌ ${tableName}: ${error.message} (REFERENCED BUT MISSING)`);
  ...
  unusedTables.forEach(table => {
    ...
    console.log(`   - ${table}: ${tableInfo?.records} records (${tableInfo?.purpose})`);
  });
