...
async function analyzeExistingReplies() {
  try {
    ...
    console.log(`📊 Found ${people?.length || 0} people with replies to analyze`);
    ...
    for (const person of people) {
      try {
        ...
        const response = await fetch(`${supabaseUrl}/functions/v1/analyze-reply`, {
          method: 'POST',
          headers: {
        ...
        if (result.success) {
          console.log(`✅ ${person.name}: ${result.analysis.replyType} (${(result.analysis.confidence * 100).toFixed(1)}% confidence)`);
        } else {
