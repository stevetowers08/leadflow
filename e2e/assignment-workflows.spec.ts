...
test.describe('User Assignment E2E Tests', () => {
  ...
  test.describe('Single Lead Assignment', () => {
    test('should assign a lead to a user', async ({ page }) => {
      ...
      await expect(leadRow.locator('[data-testid="lead-owner"]')).toContainText(
        'Unassigned'
      );
      ...
      // Verify assignment
      await expect(
        page.locator('[data-testid="assignment-success-message"]')
      ).toBeVisible();
      await expect(leadRow.locator('[data-testid="lead-owner"]')).toContainText(
        'John Doe'
      );
    ...
    test('should unassign a lead', async ({ page }) => {
      ...
      // Verify unassignment
      await expect(
        page.locator('[data-testid="unassignment-success-message"]')
      ).toBeVisible();
      await expect(leadRow.locator('[data-testid="lead-owner"]')).toContainText(
        'Unassigned'
      );
    ...
    test('should show assignment history', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="assignment-history-modal"]')
      ).toBeVisible();
      await expect(page.locator('[data-testid="history-entry"]')).toHaveCount(
        2
      );
  ...
  test.describe('Bulk Lead Assignment', () => {
    test('should assign multiple leads to a user', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="bulk-assignment-dialog"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="selected-count"]')
      ).toContainText('3 leads');
      ...
      await expect(
        page.locator('[data-testid="bulk-assignment-success"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="bulk-assignment-success"]')
      ).toContainText('Successfully assigned 3 of 3 leads');
      ...
      await expect(
        page
          ...
          .first()
          .locator('[data-testid="lead-owner"]')
    ...
    test('should handle partial bulk assignment failure', async ({ page }) => {
      ...
      await page.evaluate(() => {
        ...
        const leadRow = document.querySelector(
          '[data-testid="lead-row"]:nth-child(2)'
        );
        if (leadRow) {
          leadRow.remove();
        }
      ...
      await expect(
        page.locator('[data-testid="bulk-assignment-warning"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="bulk-assignment-warning"]')
      ).toContainText('1 leads could not be assigned');
    });

    test('should show progress indicator during bulk assignment', async ({
      page,
    }) => {
      await page.goto(`${BASE_URL}/leads`);
      ...
      await expect(
        page.locator('[data-testid="bulk-assignment-progress"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="bulk-assignment-progress"]')
      ).toContainText('Assigning leads...');
  ...
  test.describe('Company Assignment', () => {
    test('should assign a company to a user', async ({ page }) => {
      ...
      await expect(
        companyRow.locator('[data-testid="company-owner"]')
    ...
    test('should require admin permissions for company assignment', async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="permission-denied-message"]')
      ).toContainText('Only administrators can assign company ownership');
    });
  });
  ...
  test.describe('Assignment Management Panel', () => {
    test('should display assignment statistics', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="total-assigned-card"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="unassigned-card"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="team-members-card"]')
      ).toBeVisible();

      // Verify statistics values
      await expect(
        page.locator('[data-testid="total-assigned-count"]')
      ).toContainText(/\d+/);
      await expect(
        page.locator('[data-testid="unassigned-count"]')
      ).toContainText(/\d+/);
      await expect(
        page.locator('[data-testid="team-members-count"]')
      ).toContainText(/\d+/);
    });
    ...
    test('should display assignments by user table', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="assignments-table"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="table-header-user"]')
      ).toContainText('User');
      await expect(
        page.locator('[data-testid="table-header-role"]')
      ).toContainText('Role');
      await expect(
        page.locator('[data-testid="table-header-count"]')
      ).toContainText('Assigned Items');
      await expect(
        page.locator('[data-testid="table-header-actions"]')
      ).toContainText('Actions');

      // Verify user rows are displayed
      await expect(page.locator('[data-testid="user-row"]')).toHaveCount(
        testUsers.length
      );
    });
    ...
    test('should reassign user records', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="reassignment-dialog"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="from-user-display"]')
      ).toContainText('John Doe');
      ...
      await expect(
        page.locator('[data-testid="reassignment-success-message"]')
      ).toBeVisible();
    });
    ...
    test('should refresh assignment data', async ({ page }) => {
      ...
      const initialCount = await page
        .locator('[data-testid="total-assigned-count"]')
        .textContent();
      ...
      await expect(
        page.locator('[data-testid="total-assigned-count"]')
      ).not.toContainText(initialCount || '');
    });
    ...
    test('should show permission denied for non-admin users', async ({
      ...
      await expect(
        page.locator('[data-testid="permission-denied-message"]')
      ).toContainText("You don't have permission to manage assignments");
    });
  });
  ...
  test.describe('Assignment Workflow Integration', () => {
    test('should complete full assignment workflow', async ({ page }) => {
      ...
      const johnRow = page
        ...
        .filter({ hasText: 'John Doe' });
      await expect(
        johnRow.locator('[data-testid="assignment-count"]')
      ).toContainText('2');
      ...
      await expect(
        janeRow.locator('[data-testid="assignment-count"]')
      ).toContainText('1');
      ...
      await expect(
        bobRow.locator('[data-testid="assignment-count"]')
      ).toContainText('2');
    ...
    test('should handle concurrent assignment attempts', async ({
      page,
      context,
    }) => {
      ...
      const successMessages = await Promise.all([
        ...
        secondPage
          .locator('[data-testid="assignment-success-message"]')
          .count(),
      ]);
      ...
      const conflictMessages = await Promise.all([
        ...
        secondPage
          .locator('[data-testid="assignment-conflict-message"]')
          .count(),
      ]);
