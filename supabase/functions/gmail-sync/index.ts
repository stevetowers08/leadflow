...
async function syncInboxEmails(accessToken: string, userId: string) {
  try {
    ...
    const messagesResponse = await fetch('https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=50', {
      headers: {
    ...
    for (const messageRef of messagesData.messages || []) {
      try {
        ...
        const messageResponse = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${messageRef.id}`, {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
          },
        })
...
async function syncSentEmails(accessToken: string, userId: string) {
  try {
    ...
    const messagesResponse = await fetch('https://gmail.googleapis.com/gmail/v1/users/me/messages?labelIds=SENT&maxResults=50', {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
      },
    })
    ...
    for (const messageRef of messagesData.messages || []) {
      try {
        ...
        console.error(`Failed to process sent message ${messageRef.id}:`, error)
      }
    }
...
async function storeSentEmail(emailData: any, gmailMessageId: string, userId: string) {
  const messageData = {
...
function extractEmails(emailString: string): string[] {
  const emails = emailString.split(',').map(email => extractEmail(email.trim()))
