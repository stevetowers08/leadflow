...
serve(async req => {
  ...
  try {
    ...
    if (!webhookData.conversationId && !webhookData.threadId) {
      throw new Error(
        'Missing required webhook fields: conversationId or threadId'
      );
    }
...
async function processExpandiWebhook(webhookData: any) {
  try {
    ...
    let conversation = await findConversationByLinkedInId(
      expandiData.conversationId
    );
    ...
    if (!conversation) {
      ...
      if (!person) {
        ...
        return {
          ...
          message: 'Person not found in CRM',
        };
      }
      ...
      conversation = await createConversation({
        ...
        participants: [
          expandiData.personEmail || expandiData.personName || 'Unknown',
        ],
