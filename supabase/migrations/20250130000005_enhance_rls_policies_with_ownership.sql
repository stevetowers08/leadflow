...
CREATE POLICY "Users can view companies they own or admin/owner can view all" ON public.companies FOR
SELECT USING (
    ...
    OR -- Users can view companies assigned to them
    owner_id = auth.uid()
  );
CREATE POLICY "Users can insert companies and assign to themselves" ON public.companies FOR
INSERT WITH CHECK (
    ...
    AND -- Can only assign to themselves unless admin/owner
    (
      owner_id = auth.uid()
      OR EXISTS (
        SELECT 1
...
CREATE POLICY "Users can update companies they own or admin/owner can update all" ON public.companies FOR
UPDATE USING (
    ...
    OR -- Users can update companies assigned to them
    owner_id = auth.uid()
  ) WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
    ...
    AND -- Can only reassign to themselves unless admin/owner
    (
      owner_id = auth.uid()
      OR EXISTS (
        SELECT 1
...
CREATE POLICY "Only admin/owner can delete companies" ON public.companies FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM public.user_profiles up
    WHERE up.id = auth.uid()
...
CREATE POLICY "Users can insert people and assign to themselves" ON public.people FOR
INSERT WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
      SELECT 1
...
CREATE POLICY "Users can update people they own or admin/owner can update all" ON public.people FOR
UPDATE USING (
    ...
    OR -- Users can update people assigned to them
    owner_id = auth.uid()
  ) WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
...
CREATE POLICY "Only admin/owner can delete people" ON public.people FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM public.user_profiles up
    WHERE up.id = auth.uid()
...
CREATE POLICY "Users can view jobs they own or admin/owner can view all" ON public.jobs FOR
SELECT USING (
    ...
    OR -- Users can view jobs assigned to them
    owner_id = auth.uid()
  );
CREATE POLICY "Users can insert jobs and assign to themselves" ON public.jobs FOR
INSERT WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
      SELECT 1
...
CREATE POLICY "Users can update jobs they own or admin/owner can update all" ON public.jobs FOR
UPDATE USING (
    ...
    OR -- Users can update jobs assigned to them
    owner_id = auth.uid()
  ) WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
...
CREATE POLICY "Only admin/owner can delete jobs" ON public.jobs FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM public.user_profiles up
    WHERE up.id = auth.uid()
...
CREATE POLICY "Users can view interactions for their assigned entities or admin/owner can view all" ON public.interactions FOR
SELECT USING (
    ...
    OR -- Users can view interactions for entities they own
    EXISTS (
      SELECT 1
...
CREATE POLICY "Users can insert interactions for their assigned entities" ON public.interactions FOR
INSERT WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
      SELECT 1
    ...
    AND -- Can only create interactions for entities they own
    (
      EXISTS (
        SELECT 1
        FROM public.people p
...
CREATE POLICY "Users can update interactions for their assigned entities" ON public.interactions FOR
UPDATE USING (
    ...
    OR -- Users can update interactions for entities they own
    EXISTS (
      SELECT 1
      FROM public.people p
      WHERE p.id = interactions.person_id
...
CREATE POLICY "Only admin/owner can delete interactions" ON public.interactions FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM public.user_profiles up
    WHERE up.id = auth.uid()
...
CREATE POLICY "Users can view campaigns they own or admin/owner can view all" ON public.campaigns FOR
SELECT USING (
    ...
    OR -- Users can view campaigns assigned to them
    owner_id = auth.uid()
  );
CREATE POLICY "Users can insert campaigns and assign to themselves" ON public.campaigns FOR
INSERT WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
      SELECT 1
...
CREATE POLICY "Users can update campaigns they own or admin/owner can update all" ON public.campaigns FOR
UPDATE USING (
    ...
    OR -- Users can update campaigns assigned to them
    owner_id = auth.uid()
  ) WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
...
CREATE POLICY "Only admin/owner can delete campaigns" ON public.campaigns FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM public.user_profiles up
    WHERE up.id = auth.uid()
...
CREATE POLICY "Users can view campaign participants for their campaigns or admin/owner can view all" ON public.campaign_participants FOR
SELECT USING (
    ...
    OR -- Users can view participants for campaigns they own
    EXISTS (
      SELECT 1
...
CREATE POLICY "Users can insert campaign participants for their campaigns" ON public.campaign_participants FOR
INSERT WITH CHECK (
    -- Must be authenticated and active
    EXISTS (
      SELECT 1
    ...
    AND -- Can only create participants for campaigns they own
    (
      EXISTS (
        SELECT 1
        FROM public.campaigns c
...
CREATE POLICY "Users can update campaign participants for their campaigns" ON public.campaign_participants FOR
UPDATE USING (
    ...
    OR -- Users can update participants for campaigns they own
    EXISTS (
      SELECT 1
      FROM public.campaigns c
      WHERE c.id = campaign_participants.campaign_id
...
CREATE POLICY "Only admin/owner can delete campaign participants" ON public.campaign_participants FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM public.user_profiles up
    WHERE up.id = auth.uid()
...
IF user_role IN ('owner', 'admin') THEN RETURN TRUE;
END IF;
-- Get entity owner
CASE
  entity_type
  WHEN 'companies' THEN
  SELECT owner_id INTO entity_owner_id
...
IF current_user_role IN ('owner', 'admin') THEN RETURN TRUE;
END IF;
