...
#### Design Principles
- **Solid White Backgrounds**: All cards use `bg-white` for maximum readability and consistency
- **60-30-10 Color Rule**: Balanced color distribution
- **Consistent Shadows**: `shadow-sm`, `shadow-lg` with gray tints
- **Rounded Corners**: `rounded-2xl` for modern feel
- **Hover Effects**: Scale and shadow transitions with `duration-200`
- **Clickable Cards**: All small cards open popups via `usePopupNavigation` context

#### Usage Example
```tsx
import { MetricCard, ActionCard, ModernCard } from '@/components/ui/modern-cards';
...
// Button sizes - ALL ACTION ELEMENTS USE h-9 (36px) HEIGHT
const buttonSizes = {
  sm: "px-3 py-1.5 text-sm h-8",     // Small buttons
  md: "px-4 py-2 text-base h-9",      // Standard height for all action elements
...
<SearchIconButton 
  ...
  className="border-border hover:bg-muted/50 h-8 w-8"
/>
...
<button
  ...
  className="px-2 h-8 text-sm border rounded bg-background hover:bg-muted/50 flex items-center justify-center"
>
  {sortOrder === "asc" ? "↑" : "↓"}
</button>
```
...
```tsx
// Unified Table Row with Fixed Minimum Height
<EnhancedTableRow className="min-h-[56px] px-4 border-b border-border hover:bg-muted/50 transition-colors">
  <EnhancedTableCell className="px-4 min-h-[56px] text-sm">
    Content
  </EnhancedTableCell>
</EnhancedTableRow>

// Action Button in Table Row
<button className="h-8 w-8 rounded-md border flex items-center justify-center transition-colors hover:bg-muted/50">
  <Star className="h-4 w-4" />
</button>

// Status Badge in Table Row
<div className="min-h-[56px] inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border justify-center items-center flex mx-auto text-center min-w-[80px]">
  Status
</div>
```
