...
- **Company Names**: Reduced from `text-base` to `text-sm` for improved readability
- **LinkedInAutomationModal**: Reduced from `text-2xl` to `text-lg` for consistency
- **AuthModal**: Reduced from `text-2xl` to `text-lg` for better proportion
...
- **CompanyCard**: AI score badge updated to `h-8 px-3` to match StatusBadge
- **LeadInfoCard**: AI score badge updated to `h-8 px-3` for consistency
