 ...
 */
export const authenticateAndAuthorize = (options: AuthorizationOptions = {}) => {
...
async function checkAuthorization(
  ...
  const user = req.user!;
  const { requiredRole, requiredPermission, resource, action, allowOwner, allowAdmin, allowSelf } = options;
  ...
  if (resource && action) {
    const hasResourcePermission = await checkResourcePermission(user.id, resource, action);
  ...
  if (allowOwner || allowAdmin || allowSelf) {
    ...
    if (resourceId) {
      const hasOwnership = await checkOwnership(user.id, resourceId, req.route?.path);
      if (!hasOwnership) {
        return false;
      }
    }
...
async function checkPermission(userId: string, permission: string): Promise<boolean> {
  ...
  const rolePermissions: Record<string, string[]> = {
    owner: [
      'users_view', 'users_edit', 'users_delete', 'users_invite',
      'leads_view', 'leads_edit', 'leads_delete', 'leads_export', 'leads_bulk',
      'companies_view', 'companies_edit', 'companies_delete', 'companies_export', 'companies_bulk',
      'jobs_view', 'jobs_edit', 'jobs_delete', 'jobs_export', 'jobs_bulk',
      'campaigns_view', 'campaigns_edit', 'campaigns_delete', 'campaigns_export', 'campaigns_bulk',
      'reports_view', 'reports_export',
      'settings_view', 'settings_edit',
      'billing_view', 'billing_edit'
    ],
    admin: [
      'users_view', 'users_edit', 'users_invite',
      'leads_view', 'leads_edit', 'leads_delete', 'leads_export', 'leads_bulk',
      'companies_view', 'companies_edit', 'companies_delete', 'companies_export', 'companies_bulk',
      'jobs_view', 'jobs_edit', 'jobs_delete', 'jobs_export', 'jobs_bulk',
      'campaigns_view', 'campaigns_edit', 'campaigns_delete', 'campaigns_export', 'campaigns_bulk',
    ...
    recruiter: [
      ...
      'companies_view', 'companies_edit', 'companies_export',
      'jobs_view', 'jobs_edit', 'jobs_export',
      'campaigns_view', 'campaigns_edit', 'campaigns_export',
      'reports_view'
    ],
    viewer: [
      'leads_view',
 ...
 */
async function checkOwnership(userId: string, resourceId: string, routePath?: string): Promise<boolean> {
 ...
 */
export const checkEntityAccess = (entityType: 'companies' | 'people' | 'jobs' | 'campaigns') => {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const user = req.user!;
 ...
 */
export const checkAssignmentPermission = (entityType: 'companies' | 'people' | 'jobs' | 'campaigns') => {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const user = req.user!;
      const entityId = req.params.id;
 ...
 */
export const filterDataByPermissions = (entityType: 'companies' | 'people' | 'jobs' | 'campaigns') => {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const user = req.user!;
 ...
 */
export const getUserAssignedEntities = async (userId: string, entityType: string) => {
  const { data, error } = await supabase
    .from(entityType)
    .select('*')
    .eq('owner_id', userId);
 ...
 */
export const canPerformBulkOperations = async (userId: string, resource: string): Promise<boolean> => {
