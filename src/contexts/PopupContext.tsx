...
export const PopupProvider: React.FC<PopupProviderProps> = ({ children }) => {
  ...
  const { data: leadData, isLoading: isLoadingLead, error: errorLead } = useQuery({
    ...
    queryFn: async () => {
      if (!currentId || activePopup !== 'lead') return null;
    ...
    enabled: !!currentId && activePopup === 'lead',
  ...
  const { data: companyData, isLoading: isLoadingCompany, error: errorCompany } = useQuery({
    ...
    queryFn: async () => {
      if (!currentId || activePopup !== 'company') return null;
    ...
    enabled: !!currentId && activePopup === 'company',
    staleTime: 5 * 60 * 1000, // 5 minutes
    cacheTime: 10 * 60 * 1000, // 10 minutes
  });

  // Fetch job with all related data - OPTIMIZED with join
  const { data: jobData, isLoading: isLoadingJob, error: errorJob } = useQuery({
    queryKey: ['popup-job', currentId],
    queryFn: async () => {
      if (!currentId || activePopup !== 'job') return null;
    ...
    enabled: !!currentId && activePopup === 'job',
    staleTime: 5 * 60 * 1000, // 5 minutes
    cacheTime: 10 * 60 * 1000, // 10 minutes
  });

  // Fetch related leads for company/job/lead
  const { data: relatedLeads, isLoading: isLoadingRelatedLeads, error: errorRelatedLeads } = useQuery({
    queryKey: ['popup-related-leads', currentId, activePopup, activePopup === 'lead' ? 'exclude-self' : 'include-all'],
    queryFn: async () => {
      ...
      if (activePopup === 'lead') {
        console.log('🔍 Excluding current lead from related leads:', currentId);
      ...
      if (activePopup === 'lead') {
        console.log('🔍 Related leads query result:', data?.length, 'leads found');
        console.log('🔍 Current lead ID:', currentId);
        console.log('🔍 Related lead IDs:', data?.map(l => l.id));
      }
  ...
  // Fetch related jobs for company/lead
