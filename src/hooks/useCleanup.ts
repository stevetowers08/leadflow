...
export function useCleanup() {
  ...
  const addCleanup = useCallback((id: string, cleanup: () => void, type: CleanupItem['type'] = 'custom') => {
...
export function useTimeout() {
  ...
  const setCleanupTimeout = useCallback((id: string, callback: () => void, delay: number) => {
...
export function useInterval() {
  ...
  const setCleanupInterval = useCallback((id: string, callback: () => void, delay: number) => {
...
export function useEventListener() {
  const { addCleanup, removeCleanup } = useCleanup();
  const listeners = useRef<Map<string, { element: EventTarget; event: string; handler: EventListener }>>(new Map());
...
export function useSupabaseSubscription() {
  ...
  const addCleanupSubscription = useCallback((id: string, subscription: any) => {
