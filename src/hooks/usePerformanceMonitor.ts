...
export function usePerformanceMonitor() {
  ...
  const trackQueryPerformance = useCallback((queryTime: number, isError: boolean = false) => {
  ...
  const updateMetrics = useCallback(() => {
    ...
    const errorRate = totalQueriesRef.current > 0 ? errorCountRef.current / totalQueriesRef.current : 0;
    ...
    if (avgQueryTime > PERFORMANCE_CONFIG.THRESHOLDS.QUERY_TIME) {
      newAlerts.push(`Average query time (${avgQueryTime.toFixed(0)}ms) exceeds threshold`);
    }
    
    if (cacheHitRatio < PERFORMANCE_CONFIG.THRESHOLDS.CACHE_HIT_RATIO) {
      newAlerts.push(`Cache hit ratio (${(cacheHitRatio * 100).toFixed(1)}%) below threshold`);
    }
    
    if (memoryUsage > PERFORMANCE_CONFIG.THRESHOLDS.MEMORY_USAGE) {
      newAlerts.push(`Memory usage (${(memoryUsage / 1024 / 1024).toFixed(1)}MB) exceeds threshold`);
    }
    
    if (errorRate > PERFORMANCE_CONFIG.THRESHOLDS.ERROR_RATE) {
      newAlerts.push(`Error rate (${(errorRate * 100).toFixed(1)}%) exceeds threshold`);
    }
  ...
  const optimizePerformance = useCallback(async () => {
    ...
    try {
      ...
      queries.forEach(query => {
        if (query.isStale() && query.state.dataUpdatedAt < Date.now() - 5 * 60 * 1000) {
      ...
      const criticalQueries = [
        { key: ['people'], fn: () => supabase.from('people').select('id, name, stage').limit(50) },
        { key: ['companies'], fn: () => supabase.from('companies').select('id, name, pipeline_stage').limit(50) },
        { key: ['jobs'], fn: () => supabase.from('jobs').select('id, title, priority').limit(50) },
      ];
  ...
  useEffect(() => {
    const shouldOptimize = 
      slowQueriesRef.current >= PERFORMANCE_CONFIG.OPTIMIZATION_TRIGGERS.SLOW_QUERIES ||
      metrics.errorRate >= PERFORMANCE_CONFIG.OPTIMIZATION_TRIGGERS.HIGH_ERROR_RATE ||
      metrics.cacheHitRatio <= PERFORMANCE_CONFIG.OPTIMIZATION_TRIGGERS.LOW_CACHE_HIT;
  ...
  // Periodic monitoring
  useEffect(() => {
    const interval = setInterval(updateMetrics, PERFORMANCE_CONFIG.INTERVALS.PERFORMANCE_CHECK);
...
export function usePerformanceRecommendations(metrics: PerformanceMetrics) {
  ...
  useEffect(() => {
    ...
    if (metrics.slowQueries > 0) {
      newRecommendations.push('Consider adding database indexes for frequently queried columns');
...
export function usePerformanceDashboard() {
  const { metrics, alerts, isOptimizing, optimizePerformance } = usePerformanceMonitor();
  ...
  const performanceScore = useCallback(() => {
    ...
    if (metrics.memoryUsage > PERFORMANCE_CONFIG.THRESHOLDS.MEMORY_USAGE) score -= 10;
