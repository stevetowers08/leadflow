...
const createFallbackHandlers = () => ({
  navigateToLead: (id: string, name: string) => {
    console.warn('PopupNavigationProvider not available, using fallback navigation');
...
export const useClickHandlers = () => {
  ...
  try {
    ...
    console.warn('PopupNavigationProvider not available, using fallback handlers');
    navigationHandlers = createFallbackHandlers();
  }

  const { navigateToLead, navigateToCompany, navigateToJob } = navigationHandlers;

  const createEntityClickHandler = useCallback((
    entityData: EntityClickData,
...
export const useKeyboardNavigation = () => {
  ...
  try {
    ...
    console.warn('PopupNavigationProvider not available, using fallback keyboard handlers');
    navigationHandlers = createFallbackHandlers();
  }
