...
export function useEntityData({
  ...
  console.log('🔍 useEntityData called:', {
    entityType,
    entityId,
    isOpen,
    refreshTrigger,
  ...
  const OPTIMIZED_SELECTIONS = {
    people:
      'id, name, company_id, email_address, linkedin_url, employee_location, company_role, lead_score, stage, connected_at, last_reply_at, last_interaction_at, owner_id, created_at, confidence_level, is_favourite, lead_source',
    companies:
      'id, name, website, linkedin_url, head_office, industry, company_size, confidence_level, lead_score, score_reason, automation_active, is_favourite, created_at, priority, logo_url, owner_id, pipeline_stage',
  ...
  const entityQuery = useQuery({
    ...
    queryFn: async () => {
      console.log('🔍 useEntityData queryFn called for:', {
        entityType,
        entityId,
        userId: user?.id,
      });
  ...
  const companyQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () => reject(new Error('Company query timeout after 5 seconds')),
          5000
        );
      });
  ...
  const leadsQuery = useQuery({
    queryKey: [
      `${entityType}-leads`,
      entityQuery.data?.company_id || entityId,
      entityId,
      refreshTrigger,
      user?.id,
    ],
    queryFn: async () => {
      ...
      const companyId =
        entityType === 'company' ? entityId : entityQuery.data?.company_id;
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () => reject(new Error('Leads query timeout after 5 seconds')),
          5000
        );
      });
  ...
  const jobsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () => reject(new Error('Jobs query timeout after 5 seconds')),
          5000
        );
      });
