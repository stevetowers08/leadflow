...
const Companies = () => {
  ...
  >([]);
  const [selectedCompanyForAssignment, setSelectedCompanyForAssignment] =
  ...
  const filteredAndSortedCompanies = useMemo(() => {
    ...
    const filtered = companies.filter(company => {
      ...
      // Status filter
      const matchesStatus =
        statusFilter === 'all' || company.pipeline_stage === statusFilter;
      ...
      // User filter
      const matchesUser =
        selectedUser === 'all' || company.owner_id === selectedUser;
            ...
            <div onClick={e => e.stopPropagation()}>
              <FavoriteToggle
                ...
                onToggle={isFavorite => {
                  setCompanies(prev =>
                    prev.map(c =>
                      c.id === row.id ? { ...c, is_favourite: isFavorite } : c
                    )
                  );
                }}
                size='sm'
        ...
        <div className='flex items-center justify-center'>
          <AIScoreBadge
            leadData={{
              ...
              company: row.name || '',
              role: '',
              location: row.head_office || '',
            }}
            initialScore={row.lead_score ? parseInt(row.lead_score) : undefined}
          />
        </div>
      ),
    },
                  ...
                  <div className='flex items-center justify-center'>
                    <AIScoreBadge
                      leadData={{
                        ...
                        role: '',
                        location: company.head_office || '',
                      }}
                      initialScore={
                        company.lead_score
                          ? parseInt(company.lead_score)
                          : undefined
                      }
                    />
                  </div>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const { data, error } = await supabase
                        .from('companies')
                        .update({
                          owner_id: selectedCompanyForAssignment.owner_id,
                        })
