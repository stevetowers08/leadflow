...
/**
 * Contacts - Unified System Implementation
 *
 * Features:
...
// Define status arrays as constants - PDR Section 7: processing, active, replied_manual
const CONTACTS_STATUS_OPTIONS: string[] = [
  'processing', // AI enrichment in progress
  'active', // In sequence/automation active
  'replied_manual', // Lead replied, automation paused
];
...
const ContactsContent: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        // Query leads table per PDR Section 7
        ...
        // Check for errors with safe property access (2025 best practice)
        if (contactsResult.error) {
          const error = contactsResult.error;
          // Safe error logging - only log properties that exist
          if (process.env.NODE_ENV === 'development') {
            console.error('❌ Contacts query error:', {
              ...
              error, // Include full error object for debugging
            });
          }
          throw error;
        }
        ...
        if (usersResult.error) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.error('❌ User profiles query error:', {
              ...(error.message && { message: error.message }),
              ...(error.code && { code: error.code }),
              ...(error.details && { details: error.details }),
              ...(error.hint && { hint: error.hint }),
        ...
        // Use leads data directly - PDR Section 7 structure
        // Map to Contact interface for compatibility with existing components
        const normalizedContacts = (contactsResult.data || []).map(
          (lead: Record<string, unknown>) => {
            const firstName = lead.first_name as string || '';
            const lastName = lead.last_name as string || '';
            const fullName = `${firstName} ${lastName}`.trim() || 'Unknown';
            
            // Map PDR field names to Contact interface for UI compatibility
            return {
              id: lead.id,
              name: fullName,
              ...
              people_stage: lead.status || 'processing',
              owner_id: lead.owner_id || lead.user_id || null,
              // Include all PDR fields for reference
              first_name: lead.first_name,
              last_name: lead.last_name,
              email: lead.email,
              company: lead.company,
              job_title: lead.job_title,
              scan_image_url: lead.scan_image_url,
              quality_rank: lead.quality_rank,
        ...
        return {
          contacts: normalizedContacts as Contact[],
          users: usersResult.data || [],
        };
      } catch (error) {
        // Safe error handling - log only in development
        if (process.env.NODE_ENV === 'development') {
  ...
  // Extract contacts from query result
  const contacts = contactsData?.contacts || [];
  const loading = contactsLoading;
  const error = contactsError ? getErrorMessage(contactsError) : null;
  ...
  useEffect(() => {
    if (contactsError) {
      const errorMessage = getErrorMessage(contactsError);
      toast({
        title: 'Error Loading Leads',
        description: errorMessage || 'Failed to load leads table. Please try refreshing the page.',
        variant: 'destructive',
        duration: 5000,
      });
    }
  ...
  const filteredContacts = useMemo(() => {
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter(
        contact => contact.people_stage === statusFilter
      );
    }

    // User filter
    if (selectedUser !== 'all') {
      filtered = filtered.filter(contact => contact.owner_id === selectedUser);
    }
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    if (result.successCount > 0) {
      ...
      // Trigger refresh via React Query
      queryClient.invalidateQueries({ queryKey: ['contacts-page'] });
    }
  }, [bulkSelection, filteredContacts, toast, queryClient]);
  ...
  }, [bulkSelection, filteredContacts, toast]);
  ...
  }, [bulkSelection, filteredContacts, router, toast]);
  ...
  const handlePersonUpdate = useCallback(() => {
    // Refresh the contacts data via React Query
    queryClient.invalidateQueries({ queryKey: ['contacts-page'] });
  }, [queryClient]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'a') {
        e.preventDefault();
        const allIds = paginatedContacts.map(p => p.id);
  ...
  }, [bulkSelection, paginatedContacts]);
  ...
  const renderStatusDropdown = useCallback(
    (_: unknown, person: Person) => (
      <StatusDropdown
        entityId={person.id}
        entityType='leads'
        currentStatus={person.people_stage || 'processing'}
        availableStatuses={CONTACTS_STATUS_OPTIONS}
        variant='cell'
  ...
  const columns: ColumnConfig<Person>[] = useMemo(
    () => [
          ...
          <div className='flex items-center justify-center' data-bulk-checkbox>
            <Checkbox
              checked={
                ...
                paginatedContacts.every(p => bulkSelection.isSelected(p.id))
              }
              onCheckedChange={checked => {
                if (checked) {
                  bulkSelection.selectAll(paginatedContacts.map(p => p.id));
                } else {
                  bulkSelection.deselectAll();
                }
              }}
      ...
      },
      {
        ...
        cellType: 'regular',
        render: (_, person) => (
          <div className='font-semibold text-foreground whitespace-nowrap overflow-hidden text-ellipsis min-w-0'>
            {person.name || '-'}
          </div>
        ),
      },
  ...
  const stats = useMemo(
    () => [
      ...
      {
        icon: Zap,
        value: contacts.filter(p =>
          ['active', 'replied_manual'].includes(
            p.people_stage || ''
          )
        ).length,
        label: 'active',
      ...
      {
        icon: Target,
        value: contacts.filter(p => p.people_stage === 'processing').length,
        label: 'processing',
      },
      {
        icon: CheckCircle,
        value: contacts.filter(p => p.people_stage === 'replied_manual')
          .length,
        label: 'replied',
      },
    ],
