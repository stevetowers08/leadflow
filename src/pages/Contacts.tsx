...
const ContactsContent: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // Normalize the data to match our Contact interface
      const normalizedContacts = (contactsResult.data || []).map(
        (contact: Record<string, unknown>) => ({
          ...contact,
          company_name:
            (contact.companies as Record<string, unknown>)?.name || null,
          company_website:
            (contact.companies as Record<string, unknown>)?.website || null,
        })
      );
  ...
  const stats = useMemo(
    () => [
      ...
      {
        icon: Target,
        value: contacts.filter(p => p.people_stage === 'new_lead').length,
        label: getStatusDisplayText('new') + ' leads',
      },
      {
        icon: CheckCircle,
        value: contacts.filter(p => p.people_stage === 'meeting_scheduled')
          .length,
