...
export const filterItems = <T extends FilterableItem>(
  ...
  return items.filter(item => {
    const { searchTerm, statusFilter, showFavoritesOnly, selectedUser } =
      filters;
    ...
    // Favorites filter
    const isFavorite =
      toBoolean(item.favourite) || toBoolean(item.is_favourite);
    const matchesFavorites = !showFavoritesOnly || isFavorite;

    // User filter
    const matchesUser =
      selectedUser === 'all' || item.owner_id === selectedUser;
