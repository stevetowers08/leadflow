 ...
 */
export const getScoringInfo = (type: ScoringType, value: string | number): ScoringInfo => {
  switch (type) {
    case 'priority':
      return PRIORITY_LEVELS[value as PriorityLevel] || PRIORITY_LEVELS['MEDIUM'];
    
    case 'lead_score':
      return LEAD_SCORE_LEVELS[value as LeadScoreLevel] || LEAD_SCORE_LEVELS['Medium'];
    
    case 'company_score':
      return getCompanyScoreInfo(value);
    
    case 'job_score':
      return getJobScoreInfo(typeof value === 'number' ? value : parseInt(value.toString()) || 0);
 ...
 */
export const convertLevelToScore = (level: PriorityLevel | LeadScoreLevel): number => {
  switch (level) {
