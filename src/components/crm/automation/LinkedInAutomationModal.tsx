  ...
  useEffect(() => {
    if (
      ...
      selectedLeads.forEach(lead => {
        const hasLinkedInMessage =
          lead['LinkedIn Request Message'] &&
          lead['LinkedIn Request Message'].trim();
  ...
  const sendToWebhook = async (lead: Lead, message: string) => {
    ...
    const webhookPayload = {
      ...
      lead: {
        ...
        automationStatus:
          lead.automation_status_enum || lead['Automation Status'],
    ...
    try {
      ...
      try {
        ...
        if (failed > 0) {
          console.error(
            'Failed webhooks:',
            webhookResults.filter(result => result.status === 'rejected')
          );
          toast({
            title: 'Warning',
        ...
        <div className='bg-white rounded-xl shadow-2xl max-w-md w-full p-6'>
          <div className='text-center'>
            ...
            <p className='text-sm text-gray-600 mb-4'>
              No valid leads selected for automation.
            </p>
            <button
              onClick={onClose}
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-sm text-gray-600 mt-1'>
                ...
                {selectedLeads.length !== 1 ? 's' : ''} selected for automation
                {jobTitle && (
                ...
                <div className='space-y-4'>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={e =>
                        handleMessageChange(lead.id, e.target.value)
                      }
                    ...
                    />
                    <div className='text-xs text-gray-500 mt-1'>
                      This message will be sent when requesting to connect on
                      LinkedIn
                    </div>
                  </div>
                    ...
                    <textarea
                      ...
                      value={
                        (lead['LinkedIn Connected Message'] &&
                          lead['LinkedIn Connected Message'].trim()) ||
                        `Thank you for connecting! I'd love to learn more about your experience at ${lead.Company || 'your company'}.`
                      }
                      readOnly
                      style={{ minHeight: '80px' }}
                    />
                    <div className='text-xs text-gray-500 mt-1'>
                      This message will be sent automatically after they accept
                      your connection
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      value={
                        (lead['LinkedIn Follow Up Message'] &&
                          lead['LinkedIn Follow Up Message'].trim()) ||
                        `Following up on our connection. I hope you're doing well!`
                      }
                      readOnly
                      style={{ minHeight: '80px' }}
                    />
                    <div className='text-xs text-gray-500 mt-1'>
                      This message will be sent as a follow-up after the initial
                      connection
                    </div>
                  </div>
                </div>
          ...
          <div className='flex items-center justify-between'>
            <div className='text-sm text-gray-600'>
              <span className='font-medium'>
                {selectedLeads.filter(lead => lead['LinkedIn URL']).length} of{' '}
                {selectedLeads.length}
              </span>{' '}
              leads have LinkedIn profiles
            </div>
