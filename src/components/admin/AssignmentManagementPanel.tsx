...
import { Users, AlertTriangle, CheckCircle, XCircle, RefreshCw, Trash2 } from 'lucide-react';
  ...
  if (!canManageAssignments) {
    return (
      <Alert>
        ...
        <AlertDescription>
          You don't have permission to manage assignments. Only administrators and owners can access this panel.
        </AlertDescription>
      </Alert>
    );
  }
              ...
              {stats?.byUser.map((userStat) => {
                ...
                return (
                  <TableRow key={userStat.userId}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{member?.role || 'Unknown'}</Badge>
                    </TableCell>
          ...
          <div className="space-y-4">
            ...
            {newOwnerId && (
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  This will transfer all assignments from <strong>{getSelectedUserName()}</strong> to <strong>{getNewOwnerName()}</strong>. 
