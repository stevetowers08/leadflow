...
import { useAIJobSummary, useAISupabaseJobSummary, useAILeadScoring, useAILeadOptimization } from '../hooks/useAI';
...
import { Loader2, Sparkles, TrendingUp, Users, Clock, DollarSign, MapPin } from 'lucide-react';
...
export function JobSummaryGenerator({ jobData, onSummaryGenerated, className }: JobSummaryGeneratorProps) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm text-gray-600">
            ...
            {jobData.salary && <div><strong>Salary:</strong> {jobData.salary}</div>}
          </div>
        </div>
          ...
          <div className="space-y-3 pt-4 border-t">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <Badge variant={lastResult.urgency_level === 'high' ? 'destructive' : 
              ...
              <div>
                ...
                <Badge variant={lastResult.market_demand === 'high' ? 'default' : 
            ...
            <div>
              ...
              <div className="text-sm text-gray-700">{lastResult.ideal_candidate}</div>
            </div>
          </div>
        )}
      </CardContent>
...
export function BatchJobSummarization({ onComplete, className }: BatchJobSummarizationProps) {
  const { 
    processWorkflow, 
  ...
  const [jobsNeedingSummarization, setJobsNeedingSummarization] = useState<any[]>([]);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <Button 
            ...
            disabled={isLoading || isProcessing || jobsNeedingSummarization.length === 0}
            className="w-full"
          >
          ...
          <div className="space-y-2 pt-4 border-t">
            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div className="text-center">
                <div className="font-medium text-green-600">{workflowResults.processed}</div>
                <div className="text-gray-500">Processed</div>
              </div>
              <div className="text-center">
                <div className="font-medium text-blue-600">{workflowResults.updated}</div>
                <div className="text-gray-500">Updated</div>
              </div>
              <div className="text-center">
                <div className="font-medium text-red-600">{workflowResults.errors.length}</div>
...
export function LeadScoring({ leadData, onScoreGenerated, className }: LeadScoringProps) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm text-gray-600">
            ...
            <div><strong>Role:</strong> {leadData.role}</div>
            <div><strong>Location:</strong> {leadData.location}</div>
            {leadData.experience && <div><strong>Experience:</strong> {leadData.experience}</div>}
            {leadData.industry && <div><strong>Industry:</strong> {leadData.industry}</div>}
            {leadData.company_size && <div><strong>Company Size:</strong> {leadData.company_size}</div>}
          </div>
        </div>
        ...
        <Button 
          ...
          className="w-full"
        >
          {(isLoading || isScoring) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          Calculate AI Score
        </Button>
        ...
        {lastScore && (
          <div className="space-y-3 pt-4 border-t">
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-600">{lastScore.score}</div>
            ...
            <div className="text-sm">
              <div className="font-medium mb-2">Confidence: {Math.round(lastScore.confidence * 100)}%</div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <span>{Math.round(lastScore.factors.company_size * 100)}%</span>
                </div>
                <div className="flex justify-between">
                  <span>Industry Match:</span>
                  <span>{Math.round(lastScore.factors.industry_match * 100)}%</span>
                </div>
                <div className="flex justify-between">
                  <span>Role Seniority:</span>
                  <span>{Math.round(lastScore.factors.role_seniority * 100)}%</span>
                </div>
                <div className="flex justify-between">
                  <span>Location Match:</span>
                  <span>{Math.round(lastScore.factors.location_match * 100)}%</span>
                </div>
              </div>
            </div>
          </div>
...
export function LeadOptimization({ leadData, onOptimizationGenerated, className }: LeadOptimizationProps) {
  const { optimizeOutreach, isLoading, error, lastOptimization } = useAILeadOptimization({
    enableCaching: true
  });
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm text-gray-600">
            ...
            <div><strong>Role:</strong> {leadData.role}</div>
            {leadData.industry && <div><strong>Industry:</strong> {leadData.industry}</div>}
            {leadData.previous_interactions && leadData.previous_interactions.length > 0 && (
              <div><strong>Previous Interactions:</strong> {leadData.previous_interactions.join(', ')}</div>
            )}
          </div>
        </div>
        ...
        <Button 
          ...
          className="w-full"
        >
          {(isLoading || isOptimizing) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          Optimize Outreach
        </Button>
          ...
          <div className="space-y-3 pt-4 border-t">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <Badge variant={lastOptimization.priority_level === 'high' ? 'destructive' : 
                               lastOptimization.priority_level === 'medium' ? 'default' : 'secondary'}>
                  {lastOptimization.priority_level}
                </Badge>
              </div>
              <div>
                <div className="font-medium">Response Rate:</div>
                <div className="text-blue-600 font-medium">{lastOptimization.estimated_response_rate}%</div>
              </div>
            </div>

            <div>
              <div className="text-sm font-medium mb-2">Best Contact Method:</div>
            ...
            <div>
              ...
              <div className="text-sm text-gray-700">{lastOptimization.optimal_timing}</div>
            </div>
