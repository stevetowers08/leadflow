  ...
  const runAllTests = async () => {
    ...
    try {
      ...
      try {
        ...
        addResult('Supabase Connection', 'error', `Connection failed: ${err}`, {
          error: err,
        });
      }
      ...
      if (internalAvailable) {
        try {
          const testQuery =
            await dataAwareGeminiChatService['queryRelevantData'](
              'test companies'
            );
          addResult(
            'Data Query Test',
          ...
          addResult('Data Query Test', 'error', `Query failed: ${err}`, {
            error: err,
          });
        }
      }
      ...
      try {
        ...
        addResult('MCP Service', 'error', `MCP test failed: ${err}`, {
          error: err,
        });
      }
      ...
      if (internalAvailable) {
        try {
          ...
          addResult('Gemini API Test', 'error', `Gemini API failed: ${err}`, {
            error: err,
          });
        }
      }
      ...
      <CardContent className='space-y-4'>
        ...
        <div className='mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg'>
          ...
          <div className='text-sm text-blue-800 space-y-1'>
            <div>
              <strong>Internal Mode:</strong> Switch to "Internal (Data-Aware)"
              and ask: "How many companies do we have?"
            </div>
            <div>
              <strong>MCP Mode:</strong> Switch to "MCP (External)" and ask:
              "Hello, can you help me?"
            </div>
            <div>
              <strong>Data Query:</strong> Ask: "Show me all tech companies" or
