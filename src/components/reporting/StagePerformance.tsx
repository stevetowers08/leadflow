...
const StagePerformance: React.FC<StagePerformanceProps> = ({ peopleByStage, companiesByStage }) => {
  // Define stage colors and order
  const stageConfig = {
    'new': { color: 'bg-gray-500', label: 'New' },
    'connection_requested': { color: 'bg-blue-500', label: 'Connection Requested' },
          ...
          <div>
            ...
            <div className="space-y-3">
              {peopleByStage.map((stage) => {
                const config = stageConfig[stage.stage as keyof typeof stageConfig] || { color: 'bg-gray-500', label: stage.stage };
                const percentage = totalPeople > 0 ? (stage.count / totalPeople) * 100 : 0;
                
                return (
                  <div key={stage.stage} className="space-y-1">
                    <div className="flex justify-between text-sm">
                      <span className="font-medium text-gray-700">{config.label}</span>
                      <span className="text-gray-500">{stage.count} ({percentage.toFixed(1)}%)</span>
                    </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {companiesByStage.map((stage) => {
                const config = stageConfig[stage.stage as keyof typeof stageConfig] || { color: 'bg-gray-500', label: stage.stage };
                const percentage = totalCompanies > 0 ? (stage.count / totalCompanies) * 100 : 0;
