...
const AutomationFunnel: React.FC<AutomationFunnelProps> = ({ peopleByStage, companiesByStage }) => {
  // Calculate funnel metrics
  const totalPeople = peopleByStage.reduce((sum, stage) => sum + stage.count, 0);
  const totalCompanies = companiesByStage.reduce((sum, stage) => sum + stage.count, 0);
  
  // Calculate conversion rates
  const newPeople = peopleByStage.find(s => s.stage === 'new')?.count || 0;
  const connectedPeople = peopleByStage.find(s => s.stage === 'connected')?.count || 0;
  const messagedPeople = peopleByStage.find(s => s.stage === 'messaged')?.count || 0;
  const repliedPeople = peopleByStage.find(s => s.stage === 'replied')?.count || 0;
  const meetingBookedPeople = peopleByStage.find(s => s.stage === 'meeting_booked')?.count || 0;
  ...
  const funnelSteps = [
    ...
    },
    {
      ...
      percentage: connectedPeople > 0 ? (messagedPeople / connectedPeople) * 100 : 0,
      color: 'bg-purple-500',
    ...
    },
    {
      name: 'Replied',
      count: repliedPeople,
      percentage: messagedPeople > 0 ? (repliedPeople / messagedPeople) * 100 : 0,
      color: 'bg-orange-500',
    ...
    },
    {
      ...
      percentage: repliedPeople > 0 ? (meetingBookedPeople / repliedPeople) * 100 : 0,
      ...
      <CardContent>
        <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 ${step.bgColor} rounded-lg`}>
                        <IconComponent className={`h-4 w-4 ${step.textColor}`} />
                      </div>
          ...
          <div className="pt-6 border-t">
            <h4 className="font-semibold text-gray-900 mb-4">Conversion Rates</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {funnelSteps.slice(1).map((step, index) => (
                <div key={step.name} className="text-center p-3 bg-gray-50 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">
                  {newPeople > 0 ? ((meetingBookedPeople / newPeople) * 100).toFixed(1) : 0}%
                </div>
                <div className="text-sm text-purple-700">Overall Conversion</div>
              </div>
            </div>
          </div>
        </div>
