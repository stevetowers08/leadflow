  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'monospace' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div>VITE_SUPABASE_URL: {import.meta.env.VITE_SUPABASE_URL ? '✅ Present' : '❌ Missing'}</div>
        <div>VITE_SUPABASE_ANON_KEY: {import.meta.env.VITE_SUPABASE_ANON_KEY ? '✅ Present' : '❌ Missing'}</div>
      </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        <h3>Client Info:</h3>
        <div>Supabase client: {supabase ? '✅ Initialized' : '❌ Not initialized'}</div>
      </div>
    </div>
  );
}
