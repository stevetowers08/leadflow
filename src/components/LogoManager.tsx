...
export const LogoManager = () => {
  ...
  const [autoUpdateProgress, setAutoUpdateProgress] = useState<{current: number, total: number} | null>(null);
    ...
    try {
      ...
      for (let i = 0; i < companies.length; i++) {
        ...
        try {
          ...
          if (company.profile_image_url && company.profile_image_url.includes('logo.clearbit.com')) {
            skippedCount++;
            continue;
          }
    ...
    try {
      ...
      for (let i = 0; i < companies.length; i++) {
        ...
        try {
          ...
          if (company.profile_image_url && company.profile_image_url.includes('media.licdn.com')) {
            const logoUrl = getClearbitLogo(company.name, company.website);
            if (logoUrl) {
              const isValid = await testLogoUrl(logoUrl);
              if (isValid) {
                await updateCompanyLogo(company.id, logoUrl);
                replacedCount++;
                console.log(`Replaced LinkedIn logo for ${company.name} with Clearbit URL: ${logoUrl}`);
              } else {
                errorCount++;
              }
            } else {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Button onClick={handleAutoUpdate} disabled={loading || companies.length === 0}>
              <RefreshCw className="h-4 w-4 mr-2" />
              Auto-Update All Logos
            </Button>
            <Button onClick={handleReplaceLinkedInLogos} disabled={loading || companies.length === 0} variant="outline">
          ...
          {autoUpdateProgress && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-blue-900">Updating Logos...</span>
          ...
          <div className="text-sm text-gray-600">
            <p>• <strong>Auto-Update:</strong> Fetches logos from Clearbit API using company websites (free, no credentials needed)</p>
            <p>• <strong>Replace LinkedIn Logos:</strong> Replaces problematic LinkedIn URLs with reliable Clearbit URLs</p>
            <p>• <strong>Manual Update:</strong> Set custom logo URLs for specific companies</p>
            <p>• <strong>Fallback:</strong> Companies without logos show initials</p>
            <p>• <strong>Companies Loaded:</strong> {companies.length} companies found</p>
          </div>
        </CardContent>
      </Card>
      ...
      {loading && companies.length === 0 ? (
                  ...
                  <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {company.profile_image_url ? (
                      <img 
                        ...
                        onLoad={() => {
                          console.log(`Successfully loaded logo for ${company.name}: ${company.profile_image_url}`);
                        }}
                      />
                    ) : null}
                    <div 
        ...
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-gray-500">No companies were found in the database.</p>
          </CardContent>
        </Card>
      )}
      ...
      {selectedCompany && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleManualUpdate} disabled={loading || !manualUrl.trim()}>
                Update Logo
              </Button>
            ...
            {selectedCompany.website && (
              <div className="text-sm text-gray-600">
                <p><strong>Suggested Clearbit URL:</strong></p>
                <p className="font-mono text-xs bg-gray-100 p-2 rounded">
                  {getClearbitLogo(selectedCompany.name, selectedCompany.website)}
                </p>
              </div>
            )}
          </CardContent>
