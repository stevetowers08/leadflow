...
import { Users, UserCheck, AlertTriangle, CheckCircle, XCircle } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { AssignmentService, TeamMember, BulkAssignmentResult } from '@/services/assignmentService';
...
export const BulkAssignmentDialog: React.FC<BulkAssignmentDialogProps> = ({
  ...
  const entityTypeLabel = entityType === 'people' ? 'leads' : entityType === 'companies' ? 'companies' : 'jobs';
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="text-sm font-medium">Selected {entityTypeLabel}:</div>
            ...
            <Alert className={assignmentResult.success ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}>
              <div className="flex items-center gap-2">
                ...
                <AlertDescription className={assignmentResult.success ? "text-green-800" : "text-red-800"}>
                  {assignmentResult.success ? (
                    <>
                      Successfully assigned {assignmentResult.updated_count} of {assignmentResult.total_requested} {entityTypeLabel}
                      {assignmentResult.invalid_entities.length > 0 && (
                        <div className="text-sm mt-1">
                          {assignmentResult.invalid_entities.length} {entityTypeLabel} could not be assigned
                        </div>
                      )}
                    </>
                  ) : (
          ...
          {selectedOwnerId && (
            <div className="p-3 bg-muted/20 rounded-lg border">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-muted-foreground">Will assign {entityCount} {entityTypeLabel} to</span>
          ...
          {entityCount > 50 && (
            <Alert>
              ...
              <AlertDescription>
                You are assigning a large number of {entityTypeLabel}. This operation may take a moment to complete.
              </AlertDescription>
            </Alert>
          )}
        </div>
