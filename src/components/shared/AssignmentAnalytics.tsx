...
export const AssignmentAnalytics: React.FC<AssignmentAnalyticsProps> = ({
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      const activityData: UserActivity[] = statsData.byUser
        .map(userStat => {
      ...
      <CardContent className='space-y-6'>
        ...
        <div className='space-y-2'>
          <div className='flex items-center justify-between'>
            ...
            <div
              className={`flex items-center gap-1 ${getTrendColor(metrics.assignmentTrend)}`}
            >
              {getTrendIcon(metrics.assignmentTrend)}
              <span className='text-sm capitalize'>
                {metrics.assignmentTrend}
              </span>
            </div>
          </div>
            ...
            {userActivity.slice(0, 5).map((user, index) => (
              ...
              >
                ...
                <div className='text-right'>
                  ...
                  <div className='text-xs text-muted-foreground'>
                    {formatDistanceToNow(new Date(user.lastAssignment), {
                      addSuffix: true,
                    })}
                  </div>
                </div>
