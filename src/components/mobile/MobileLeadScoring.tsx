...
const scoreRanges = [
  { min: 80, max: 100, label: 'Hot', color: 'bg-red-500', textColor: 'text-red-700', bgColor: 'bg-red-50', borderColor: 'border-red-200' },
  { min: 60, max: 79, label: 'Warm', color: 'bg-orange-500', textColor: 'text-orange-700', bgColor: 'bg-orange-50', borderColor: 'border-orange-200' },
  { min: 40, max: 59, label: 'Cool', color: 'bg-blue-500', textColor: 'text-blue-700', bgColor: 'bg-blue-50', borderColor: 'border-blue-200' },
  { min: 0, max: 39, label: 'Cold', color: 'bg-gray-500', textColor: 'text-gray-700', bgColor: 'bg-gray-50', borderColor: 'border-gray-200' }
];
...
export const MobileLeadScoring: React.FC<MobileLeadScoringProps> = ({
  ...
  const [sortBy, setSortBy] = useState<'score' | 'priority' | 'activity'>('score');
  const [showInsights, setShowInsights] = useState(false);

  // Process and categorize leads
  const processedLeads = useMemo(() => {
    return leads.map(lead => {
      const score = typeof lead.lead_score === 'string' ? parseInt(lead.lead_score) : lead.lead_score;
      const range = scoreRanges.find(r => score >= r.min && score <= r.max) || scoreRanges[3];
      
      return {
        ...lead,
        scoreRange: range,
        numericScore: score,
        priorityValue: priorityOrder[lead.priority as keyof typeof priorityOrder] || 0
      };
    });
  }, [leads]);
  ...
  const leadsByRange = useMemo(() => {
    ...
    scoreRanges.forEach(range => {
      groups[range.label] = processedLeads.filter(lead => lead.scoreRange.label === range.label);
    });
  ...
  const insights = useMemo(() => {
    ...
    const avgScore = total > 0 ? processedLeads.reduce((sum, lead) => sum + lead.numericScore, 0) / total : 0;
    const automationActive = processedLeads.filter(lead => lead.automation_active).length;
    ...
    <div className="space-y-4">
          ...
          <Card className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{insights.hotLeads}</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{insights.avgScore}</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex gap-1">
            {['score', 'priority', 'activity'].map((sort) => (
              ...
              >
                {sort === 'score' ? 'Score' : sort === 'priority' ? 'Priority' : 'Activity'}
              </Button>
            ))}
          </div>
          ...
          <Card className="mt-3 p-3">
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-red-600">{insights.hotPercentage}%</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Warm Lead %:</span>
                <span className="font-medium text-orange-600">{insights.warmPercentage}%</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Automation Active:</span>
                <span className="font-medium text-green-600">{insights.automationActive}</span>
              </div>
            </div>
          </Card>
        )}
              ...
              <div key={range.label} className="space-y-2">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-sm text-gray-900 flex items-center gap-2">
                    <div className={cn("w-2 h-2 rounded-full", range.color)}></div>
      ...
      {processedLeads.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 text-sm">No leads available for scoring</p>
        </div>
      )}
    </div>
  );
...
interface MobileLeadCardProps {
  lead: LeadScoreData & { scoreRange: any; numericScore: number; priorityValue: number };
  onLeadClick: (lead: LeadScoreData) => void;
  compact?: boolean;
}
      ...
      <CardContent className={compact ? "p-0" : "p-4"}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-shrink-0">
            ...
            {trend === 'up' && <TrendingUp className="h-3 w-3 text-green-500 mt-1" />}
            {trend === 'down' && <TrendingDown className="h-3 w-3 text-red-500 mt-1" />}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <Badge className={cn("text-xs", lead.scoreRange.bgColor, lead.scoreRange.textColor, lead.scoreRange.borderColor)}>
            ...
            {lead.last_activity && (
              <div className="flex items-center gap-1 mt-2 text-xs text-gray-500">
                <span>Last activity: {new Date(lead.last_activity).toLocaleDateString()}</span>
              </div>
            )}
          </div>
        </div>
