...
const pipelineStages = [
  { key: "automated", label: "Automated", color: "bg-blue-50 text-blue-700 border-blue-200", shortLabel: "Auto" },
  { key: "replied", label: "Replied", color: "bg-green-50 text-green-700 border-green-200", shortLabel: "Reply" },
  { key: "meeting_scheduled", label: "Meeting Scheduled", color: "bg-purple-50 text-purple-700 border-purple-200", shortLabel: "Meeting" },
  { key: "proposal_sent", label: "Proposal Sent", color: "bg-orange-50 text-orange-700 border-orange-200", shortLabel: "Proposal" },
  { key: "negotiation", label: "Negotiation", color: "bg-yellow-50 text-yellow-700 border-yellow-200", shortLabel: "Negotiate" },
  { key: "closed_won", label: "Closed Won", color: "bg-emerald-50 text-emerald-700 border-emerald-200", shortLabel: "Won" },
  { key: "closed_lost", label: "Closed Lost", color: "bg-red-50 text-red-700 border-red-200", shortLabel: "Lost" },
  { key: "on_hold", label: "On Hold", color: "bg-gray-50 text-gray-700 border-gray-200", shortLabel: "Hold" }
];
...
export const MobilePipeline: React.FC<MobilePipelineProps> = ({
  ...
  const companiesByStage = useMemo(() => {
    ...
    companies.forEach(company => {
      ...
      if (matchesSearch && company.pipeline_stage && result[company.pipeline_stage]) {
  ...
  const handleQuickAction = useCallback((action: string, company: Company) => {
    lightHaptic();
              ...
              <div key={stage.key} className="space-y-2">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-sm text-gray-900 flex items-center gap-2">
                    <span className={cn("w-2 h-2 rounded-full", stage.color.split(' ')[0])}></span>
                    {stage.label}
                  </h3>
      ...
      {Object.values(companiesByStage).flat().length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 text-sm">
            {searchTerm ? 'Try adjusting your search terms' : 'No companies in pipeline'}
          </p>
        </div>
      )}
    </div>
...
const MobileCompanyCard: React.FC<MobileCompanyCardProps> = ({
  ...
  const currentStage = pipelineStages.find(stage => stage.key === company.pipeline_stage);
  ...
  const handleQuickActionClick = useCallback((e: React.MouseEvent, action: string) => {
    e.stopPropagation();
