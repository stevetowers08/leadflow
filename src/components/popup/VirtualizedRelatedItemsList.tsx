...
const VirtualListItem = ({
  ...
  return <div style={style}>{renderItem()}</div>;
};

export const VirtualizedRelatedItemsList: React.FC<VirtualizedRelatedItemsListProps> =
  React.memo(
    ({
      title,
      items,
        ...
        <InfoCard title={title} contentSpacing='space-y-3 pt-3'>
          {shouldUseVirtualScrolling ? (
            ...
            <div className='space-y-2'>
              {items.map(item => {
                if (itemType === 'lead') {
                  const { avatarUrl, initials } = getProfileImage(
                    item.name,
                    32
                  );
                  return (
          ...
          {showAutomateButton && selectedLeads.length > 0 && (
            <div className='mt-4 pt-4 border-t border-gray-200'>
              ...
              >
                ...
                Automate {selectedLeads.length} Lead
