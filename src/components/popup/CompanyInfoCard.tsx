...
export const CompanyInfoCard: React.FC<CompanyInfoCardProps> = ({
  ...
  const handleCompanyClick = onCompanyClick
  ...
  return (
    <InfoCard
      title={showTitle ? 'Company Information' : undefined}
      contentSpacing='space-y-4 pt-1'
      showDivider={false}
    >
      ...
      <div className='grid grid-cols-3 gap-3 items-center'>
        ...
        <div
          ...
          aria-label={
            handleCompanyClick
              ? `View ${company.name} company details`
              : undefined
          }
          aria-describedby={
            handleCompanyClick ? `company-${company.id}-description` : undefined
          }
        >
          ...
          <div className='flex-shrink-0 w-12 h-12 rounded-lg overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center'>
            ...
            <Building2
              className={`w-8 h-8 text-gray-400 ${logo ? 'hidden' : ''}`}
            />
          </div>
        ...
        <div className='space-y-1'>
          ...
          <div className='text-sm text-gray-900 font-medium'>
            <div
              className={`h-7 px-2 rounded-md border text-xs font-semibold w-fit flex items-center justify-center ${getScoreBadgeClasses(company.lead_score)}`}
            >
              {company.lead_score || '-'}
            </div>
          </div>
      ...
      <div className='space-y-2'>
        ...
        {tags.length > 0 ? (
          <div className='space-y-2'>
            <TagDisplay tags={tags} size='sm' maxVisible={5} showAll={true} />
            <button
