...
const LineChart = ({ data, height = 200, showGrid = true, showValues = true }: LineChartProps) => {
  ...
  const points = data.map((point, index) => {
    ...
    const y = adjustedRange > 0 ? 160 - ((point.value - adjustedMin) / adjustedRange) * 120 : 100;
    return { x, y, ...point };
  });
  ...
  return (
    <div className="relative w-full" style={{ height }}>
      <svg width="100%" height="100%" viewBox="0 0 400 200" className="overflow-visible">
        ...
        {showGrid && (
          <>
            <text x="35" y="25" textAnchor="end" className="text-xs fill-current text-muted-foreground">
              {adjustedMax.toLocaleString()}
            </text>
            <text x="35" y="100" textAnchor="end" className="text-xs fill-current text-muted-foreground">
              {Math.round((adjustedMax + adjustedMin) / 2).toLocaleString()}
            </text>
            <text x="35" y="175" textAnchor="end" className="text-xs fill-current text-muted-foreground">
    ...
    <div className="relative w-full" style={{ height }}>
      <svg width="100%" height="100%" viewBox="0 0 400 200" className="overflow-visible">
        {data.map((item, index) => {
          ...
          const y = orientation === "horizontal" ? 20 + (index * spacing) : 180 - barHeight;
          ...
          return (
            <g key={index}>
              ...
              {showValues && (
                <text
                  x={orientation === "horizontal" ? x + barHeight + 8 : x + barWidth / 2}
                  y={orientation === "horizontal" ? y + barWidth / 2 + 4 : y - 8}
...
const DonutChart = ({ 
  ...
  const segments = data.map((item, index) => {
    ...
    const color = item.color || stageColors[item.label.toLowerCase()] || `hsl(${(index * 137.5) % 360}, 70%, 50%)`;
...
const KPICard = ({ 
  ...
  if (loading) {
    return (
      <div className={cn("p-6 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10", className)}>
...
export {
    BarChart,
    DonutChart, KPICard, LineChart, MiniChart, TrendIndicator
};
