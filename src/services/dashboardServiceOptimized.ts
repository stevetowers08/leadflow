...
export class DashboardServiceOptimized {
  ...
  static async getDashboardData(): Promise<DashboardData> {
    ...
    return safeAsync(
      async () => {
        ...
        const [peopleResult, companiesResult, jobsResult] = await Promise.race([
          dataPromise,
          timeoutPromise
        ]);
        ...
        const metrics: DashboardMetrics = {
          ...
          activeAutomations: people.filter(p => p.automation_started_at).length,
          automationSuccessRate: companies.length > 0 ? Math.round((people.filter(p => p.automation_started_at).length / companies.length) * 100) : 0,
        ...
        const dashboardData: DashboardData = {
          ...
          peopleAutomationOverTime: this.generatePeopleAutomationOverTime(people)
        };
  ...
  private static calculatePipelineBreakdown(people: any[]): Record<string, number> {
  ...
  private static generateCompaniesOverTime(companies: any[]): Array<{ date: string; newCompanies: number; automatedCompanies: number }> {
    ...
    return last7Days.map(date => ({
      ...
      automatedCompanies: companies.filter(c => c.automation_started_at && c.automation_started_at.startsWith(date)).length
    }));
  }

  private static generatePeopleAutomationOverTime(people: any[]): Array<{ date: string; peopleWithAutomation: number; automationActivity: number }> {
    const last7Days = Array.from({ length: 7 }, (_, i) => {
      const date = new Date();
      date.setDate(date.getDate() - i);
      return date.toISOString().split('T')[0];
    ...
    return last7Days.map(date => ({
      date,
      peopleWithAutomation: people.filter(p => p.automation_started_at && p.automation_started_at.startsWith(date)).length,
      automationActivity: people.filter(p => p.last_interaction_at && p.last_interaction_at.startsWith(date)).length
    }));
  }
