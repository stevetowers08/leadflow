...
import { Download, TrendingUp, TrendingDown, Flame, Zap, AlertCircle } from 'lucide-react';
...
export default function AnalyticsPage() {
  ...
  const { data: analytics, isLoading, error } = useLeadAnalytics({
    ...
    >
      <div className={cn('space-y-6', designTokens.spacing.pagePadding.full)}>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className={cn(designTokens.typography.heading.h1, 'text-3xl font-bold')}>
              Analytics & Insights
            </h1>
          ...
          <div className="flex items-center gap-4">
            <Select
              ...
              onValueChange={(value: 'this_event' | 'last_30_days' | 'all_time') =>
        ...
        {isLoading ? (
          ...
          <div className={designTokens.layout.cardGrid}>
            ...
            <StatCard
              ...
              trend={analytics.totalLeadsChange > 0 ? 'up' : analytics.totalLeadsChange < 0 ? 'down' : 'neutral'}
            />
            ...
            <StatCard
              ...
              trend={analytics.responseRateChange > 0 ? 'up' : analytics.responseRateChange < 0 ? 'down' : 'neutral'}
            />
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={analytics.workflowPerformance} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" />
                <YAxis
            ...
            <CardContent>
              <div className="space-y-4">
                {analytics.recentActivity.length === 0 ? (
                  <p className={cn(designTokens.typography.body.muted, 'text-center py-8')}>
                    No recent activity
                  </p>
                ) : (
                    ...
                    >
                      <div className="mt-1">
                        {activity.type === 'lead_captured' && (
                          <div className={cn('h-2 w-2 rounded-full', designTokens.colors.background.info)} />
                        )}
                        {activity.type === 'email_sent' && (
                          <div className={cn('h-2 w-2 rounded-full', designTokens.colors.background.success)} />
                        )}
                        {activity.type === 'email_opened' && (
                          <div className={cn('h-2 w-2 rounded-full', designTokens.colors.background.warning)} />
                        )}
                        {activity.type === 'email_replied' && (
                          <div className="h-2 w-2 rounded-full bg-purple-500" />
                        )}
                        {activity.type === 'workflow_paused' && (
                          <div className={cn('h-2 w-2 rounded-full', designTokens.colors.background.error)} />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={cn(designTokens.typography.body.default, 'font-medium')}>
                          {activity.description}
                        </p>
                        <p className={cn(designTokens.typography.body.small, 'mt-1')}>
